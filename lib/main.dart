...
class ClothPosition {
  ...
  ClothPosition({required this.clothId, this.x = 50.0, this.y = 50.0, this.scale = 1.0});
  
  Map<String, dynamic> toJson() => {'clothId': clothId, 'x': x, 'y': y, 'scale': scale};
...
class Outfit {
  ...
  Outfit({required this.id, required this.name, required this.clothesPositions, this.isFavorite = false});
  ...
  factory Outfit.fromJson(Map<String, dynamic> json) => Outfit(
    id: json['id'], 
    name: json['name'], 
    clothesPositions: (json['clothesPositions'] as List).map((e) => ClothPosition.fromJson(e)).toList(), 
...
class _MainNavigationState extends State<MainNavigation> {
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                Text(outfit.name, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                IconButton(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(builder: (context, setModalState) {
        ...
        try { 
          ...
          activePos = null; // Fix pour le 'Empty catch block'
        }
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              const Text("Toucher un habit pour le régler | Appui long pour supprimer", style: TextStyle(fontSize: 10, color: Colors.grey)),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Slider(
                          ...
                          min: 0.3,
                          max: 3.0,
                          onChanged: (value) => setModalState(() => activePos!.scale = value),
                        ),
                      ),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Row(children: [
                          IconButton(icon: Icon(outfit.isFavorite ? Icons.favorite : Icons.favorite_border, color: outfit.isFavorite ? Colors.red : Colors.grey), onPressed: () { setState(() => outfit.isFavorite = !outfit.isFavorite); _saveData(); }), 
                          IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red), onPressed: () { setState(() => myOutfits.removeWhere((o) => o.id == outfit.id)); _saveData(); })
                        ])
                      ]
                    ), 
                    const SizedBox(height: 10), 
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, cIdx) { 
                          ...
                          return Padding(
                            ...
                            child: ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(File(cloth.imagePath), width: 80, height: 100, fit: BoxFit.cover))
                          ); 
                        }
                      )
                    )
  ...
  Widget _buildStatsTab() { 
    ...
    for (var cloth in myWardrobe) {
      counts[cloth.mainCategory] = (counts[cloth.mainCategory] ?? 0) + 1; // Fix de la boucle for
    }
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween, 
                    children: [Text(entry.key), Text("${(percent * 100).toInt()}%")]
                  ), 
    ...
    if (_openedCategory == null) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) { 
          ...
          return InkWell(
            onTap: () => setState(() { _openedCategory = cat.name; _searchQuery = ""; _selectedSubFilter = null; }), 
            child: Container(
              decoration: BoxDecoration(color: Colors.indigo.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.indigo.withValues(alpha: 0.3))), 
              child: Column(
      ...
      var subCats = myWardrobe.where((c) => c.mainCategory == _openedCategory).map((c) => c.subCategory).toSet().toList(); 
      
      return Column(children: [
        AppBar(leading: IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => setState(() => _openedCategory = null)), title: Text(_openedCategory!)), 
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 15), 
          child: TextField(decoration: InputDecoration(hintText: "Rechercher...", prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(30)), contentPadding: EdgeInsets.zero), onChanged: (v) => setState(() => _searchQuery = v))
        ), 
            ...
            itemBuilder: (context, index) { 
              ...
              return Card(
                ...
                child: Stack(children: [
                  ...
                  Positioned(right: 0, child: IconButton(icon: const Icon(Icons.remove_circle, color: Colors.red), onPressed: () { setState(() { myWardrobe.removeWhere((c) => c.id == item.id); if (!myWardrobe.any((c) => c.mainCategory == _openedCategory)) _openedCategory = null; }); _saveData(); }))
                ])
              ); 
            }
          )
  ...
  // --- AJOUTER HABIT (AVEC CRÉATION DE CATÉGORIES) ---
  ...
  Future<void> _pickImage(ImageSource s) async { 
    ...
    if (f != null) _showAddEntrySheet(File(f.path)); 
  }
        ...
        builder: (context, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Fix pour value déprécié et fonctions manquantes
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10),
                ...
                child: DropdownButton<String>(
                  ...
                  items: [
                    ...
                    const DropdownMenuItem(value: "NEW", child: Text("+ Créer une catégorie", style: TextStyle(color: Colors.indigo, fontWeight: FontWeight.bold)))
                  ], 
                  onChanged: (v) {
                    if (v == "NEW") {
              ...
              Container(
                ...
                child: DropdownButton<String>(
                  ...
                  items: [
                    ...
                    const DropdownMenuItem(value: "NEW_S", child: Text("+ Créer un type", style: TextStyle(color: Colors.indigo, fontWeight: FontWeight.bold)))
                  ], 
                  onChanged: (v) {
                    if (v == "NEW_S") {
              ...
              ElevatedButton(
                ...
                onPressed: () { 
                  setState(() => myWardrobe.add(Cloth(id: DateTime.now().toString(), imagePath: img.path, mainCategory: sCat, subCategory: sSub))); 
                  _saveData(); Navigator.pop(context); 
                }, 
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: _openedCategory == null ? AppBar(title: const Text('My Dressing'), centerTitle: true) : null,
      body: [_buildDressingContent(), _buildOutfitsTab(), _buildStatsTab()][_selectedIndex],
      bottomNavigationBar: NavigationBar(
        selectedIndex: _selectedIndex, 
        onDestinationSelected: (i) => setState(() { _selectedIndex = i; _openedCategory = null; }), 
        destinations: const [
